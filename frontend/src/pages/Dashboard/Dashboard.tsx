...
const Dashboard: React.FC = () => {
  ...
  const handleAddFood = () => {
    ...
    if (isRestaurantUser) {
      ...
      const payload = {
        item_name: newFood.name,
        description: newFood.description,
        price: price
      };

      setLoading(true);
      backendApi.addMenuItem(payload as any)
        .then(saved => {
          ...
          setNewFood({ name: '', description: '', price: '', icon: '' });
        })
        .catch((err: any) => {
          console.error('Failed to add menu item:', err);
          alert(`Failed to add menu item: ${err.response?.data || err.message}`);
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  const handleUpdateFood = () => {
    ...
    if (isRestaurantUser && editingFood.item_ID) {
      const payload = {
        item_name: newFood.name,
        description: newFood.description,
        price: price
      ...
      setLoading(true);
      backendApi.updateMenuItem(editingFood.item_ID, payload as any)
        .then(saved => {
        ...
        .catch((err: any) => {
          ...
          alert(`Failed to update menu item: ${err.response?.data || err.message}`);
        })
        .finally(() => {
          setLoading(false);
        });
  ...
  const handleDeleteFood = (foodId: number) => {
    ...
    if (isRestaurantUser && target?.item_ID) {
      if (!window.confirm('Are you sure you want to delete this menu item?')) {
        return;
      }

      setLoading(true);
      backendApi.deleteMenuItem(target.item_ID)
        .then(() => {
          setFoods(foods.filter(food => food.id !== foodId));
        })
        .catch((err: any) => {
          console.error('Failed to delete menu item:', err);
          alert(`Failed to delete menu item: ${err.response?.data || err.message}`);
        })
        .finally(() => {
          setLoading(false);
        });
