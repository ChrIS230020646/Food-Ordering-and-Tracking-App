...
import {
  ...
  LocalShipping as DeliveryFeeIcon,
        ...
        orders.map((order) => (
          <Card key={order.id} sx={{ width: '100%' }}>
            <CardContent>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', 
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    <DeliveryFeeIcon fontSize="small" color="primary" />
              ...
              {/* 如果載入配送員 profile 有錯誤，且訂單仍可被接單，顯示警告 */}
              {profileError && order.status === 'preparing' && !order.deliveryStaff && (
                <Alert 
                  severity="warning" 
                  sx={{ mb: 2 }}
                  action={
              ...
              {order.status === 'pending' && !order.deliveryStaff && (
                <>
                  ...
                  <Alert severity="info" sx={{ mb: 2 }}>
                    Waiting for restaurant to confirm this order. You can accept it once the restaurant confirms.
                  </Alert>
                </>
              )}

              
              {/* 可接單條件：餐廳已確認 (preparing)，且尚未被任何配送員接單 */}
              {order.status === 'preparing' && !order.deliveryStaff && (
                <>
                  <Divider sx={{ my: 2 }} />
                  <Button
                    variant="contained"
