...
const RestaurantOrders: React.FC<RestaurantOrdersProps> = ({
  ...
  const handleStatusUpdate = async () => {
    if (selectedOrder && statusUpdate) {
      ...
      try {
        await onUpdateOrderStatus(selectedOrder, statusUpdate);
      ...
      >
        <DialogTitle sx={{ borderBottom: 1, borderColor: 'divider', pb: 2 }}>
          ...
          {selectedOrder && (
            <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
              Order #{selectedOrder.orderid} • {selectedOrder.restaurantName}
            </Typography>
          )}
        </DialogTitle>
        ...
        <DialogContent sx={{ pt: 3 }}>
          <Alert severity="info" sx={{ mb: 3 }}>
            Current Status: <strong>{selectedOrder && getStatusLabel(selectedOrder.status)}</strong>
          </Alert>
          ...
          >
            <MenuItem value="pending">
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                ...
                <Box>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Order received, waiting to start preparation
                  </Typography>
                </Box>
              </Box>
            </MenuItem>
            <MenuItem value="preparing">
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                <FastfoodIcon color="info" />
                <Box>
                  <Typography variant="body1">Preparing</Typography>
                  <Typography variant="caption" color="text.secondary">
                    Currently preparing the order
                  </Typography>
                </Box>
              </Box>
            </MenuItem>
        ...
        <Box sx={{ p: 2, borderTop: 1, borderColor: 'divider', display: 'flex', gap: 1, justifyContent: 'flex-end' }}>
          ...
          <Button 
            ...
            variant="contained"
            disabled={updateLoading || !statusUpdate || statusUpdate === selectedOrder?.status}
            startIcon={updateLoading ? null : <UpdateIcon />}
          >
