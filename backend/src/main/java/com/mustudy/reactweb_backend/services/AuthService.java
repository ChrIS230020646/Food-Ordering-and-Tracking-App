...
public class AuthService {
    ...
    try {
        ...
        switch (request.getUserType().toLowerCase()) {
            case "customer":
                user = customerRepository.findByEmail(request.getEmail()).orElse(null);
                break;
            case "restaurant":
                // Restaurant暫時唔知點做
                user = null;
                break;
            case "delivery":
                user = deliveryStaffRepository.findByEmail(request.getEmail()).orElse(null);
                break;
            default:
        ...
        if (user instanceof Customer) {
            ...
            passwordValid = customer.getPassHash1().equals(hashedInput1) && 
                           customer.getPassHash2().equals(hashedInput2);
        } else if (user instanceof DeliveryStaff) {
            DeliveryStaff staff = (DeliveryStaff) user;
            passwordValid = staff.getPassHash1().equals(hashedInput1) && 
                           staff.getPassHash2().equals(hashedInput2);
        }
        // Restaurant之後補
