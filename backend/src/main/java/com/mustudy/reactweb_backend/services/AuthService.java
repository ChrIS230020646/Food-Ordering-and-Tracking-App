...
public class AuthService {
    ...
    private LoginResponse loginRestaurant(LoginRequest request, String hashedInput1, String hashedInput2) {
        ...
        RestaurantProfile profile = new RestaurantProfile(
            ...
            null,  // avgRating - will be loaded later if needed
            null   // ratingCount - will be loaded later if needed
        );
    ...
    private LoginResponse loginDeliveryStaff(LoginRequest request, String hashedInput1, String hashedInput2) {
        ...
        // Debug log: 檢查實際從資料庫讀到的狀態
        System.out.println("loginDeliveryStaff debug - email=" + deliveryStaff.getEmail() +
                ", status=" + deliveryStaff.getStatus() +
                ", isValidate=" + deliveryStaff.getIsValidate());

        // 對外賣員暫時不強制檢查 isValidate，避免測試帳號被誤判為停用
        // 如果未來要恢復審核機制，可以改回只在 isValidate 明確為 false 時阻擋登入
        // if (deliveryStaff.getIsValidate() != null && !deliveryStaff.getIsValidate()) {
        //     return new LoginResponse(false, "Account disabled, please contact support");
        // }
