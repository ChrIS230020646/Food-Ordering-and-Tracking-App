...
public class OrderService {
    ...
    public OrderDTO createOrder(String token, CreateOrderRequest request) {
        ...
        // 確保 deliverManId 為 null，這樣 delivery staff 才能看到這個訂單
        ...
        System.out.println("Creating order - Customer ID: " + custid + ", Restaurant ID: " + request.getRestid() + ", Status: pending");

        // Save order
        order = orderRepository.save(order);
        
        // Debug: Log order creation result
        System.out.println("Order created successfully - Order ID: " + order.getOrderid() + ", Restaurant ID: " + order.getRestid() + ", Status: " + order.getStatus() + ", DeliverManId: " + order.getDeliverManId() + ", TotalAmount: " + order.getTotalAmount());
    ...
    /**
     * Get all orders for current restaurant (Restaurant only)
     */
    public List<OrderDTO> getRestaurantOrders(String token) {
        Integer restid = getRestaurantIdFromToken(token);
        if (restid == null) {
            throw new RuntimeException("Invalid token or not a restaurant token");
        }

        // Get all orders for this restaurant, sorted by created time descending (newest first)
        // This includes all order statuses (pending, preparing, delivering, delivered, cancelled)
        List<Order> orders = orderRepository.findByRestidOrderByCreatedTimeDesc(restid);
        System.out.println("getRestaurantOrders - Restaurant ID: " + restid + ", Found " + orders.size() + " orders");
        for (Order order : orders) {
            System.out.println("  Order ID: " + order.getOrderid() + ", Status: " + order.getStatus() + ", Customer ID: " + order.getCustid() + ", TotalAmount: " + order.getTotalAmount() + ", Created: " + order.getCreatedTime());
        }
        return orders.stream()
                .map(this::convertToDTO)
                .collect(Collectors.toList());
    ...
    public List<OrderDTO> getPendingOrdersForDelivery() {
        // 對外送員而言，「可接單」包括：
        // 1. customer 剛發出的訂單（pending）
        // 2. 餐廳已確認的訂單（preparing）
        // 且尚未被任何外送員接單的訂單（deliverManId 為 null）
        List<Order.OrderStatus> statuses = new ArrayList<>();
        statuses.add(Order.OrderStatus.pending);
        statuses.add(Order.OrderStatus.preparing);
        
        System.out.println("getPendingOrdersForDelivery - Querying for orders with status: " + statuses + " and deliverManId is null");
        
        List<Order> orders = orderRepository.findByStatusInAndDeliverManIdIsNull(statuses);
        
        System.out.println("getPendingOrdersForDelivery - Found " + orders.size() + " available orders (pending + preparing)");
        for (Order order : orders) {
            System.out.println("  Order ID: " + order.getOrderid() + ", Status: " + order.getStatus() + ", DeliverManId: " + order.getDeliverManId() + ", Restaurant ID: " + order.getRestid() + ", Customer ID: " + order.getCustid() + ", TotalAmount: " + order.getTotalAmount() + ", Created: " + order.getCreatedTime());
        }
    ...
    public OrderDTO cancelOrder(String token, Integer orderId) {
        ...
        // Release the order back to preparing status so other delivery staff can accept it directly
        // Since delivery staff can only cancel orders that were already accepted (preparing status),
        // we set it back to preparing instead of pending, so other delivery staff can accept it immediately
    ...
    private OrderDTO convertToDTO(Order order) {
        ...
        // 獲取配送地址：如果 shippingAddress 為空或為 "Address not set"，則從 addressid 獲取地址信息
        String shippingAddress = order.getShippingAddress();
        if (shippingAddress == null || shippingAddress.trim().isEmpty() || 
            "Address not set".equals(shippingAddress) || "Address not set".equalsIgnoreCase(shippingAddress)) {
            // 從 addressid 獲取地址信息
            if (order.getAddressid() != null) {
                var addressOpt = customerAddressRepository.findById(order.getAddressid());
