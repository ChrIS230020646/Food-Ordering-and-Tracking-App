...
public class OrderService {
    ...
    public List<OrderDTO> getPendingOrdersForDelivery() {
        // 對外送員而言，「可接單」指餐廳已確認（preparing）且尚未被任何外送員接單的訂單
        List<Order> orders = orderRepository.findByStatusAndDeliverManIdIsNull(Order.OrderStatus.preparing);
        System.out.println("getPendingOrdersForDelivery - Found " + orders.size() + " preparing orders");
        for (Order order : orders) {
